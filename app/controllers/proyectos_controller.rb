class ProyectosController < ApplicationController
  before_action :set_proyecto, only: [:show, :edit, :update, :destroy, :add_person, :remove_person]

  # GET /proyectos
  # GET /proyectos.json
  def index
    # @proyectos = Proyecto.all
    @proyectos = Proyecto.limit(10).order('created_at DESC')
  end
  ...
  def add_person
    @proyecto.personas << Persona.new(name: params[:title])
    redirect_to proyectos_path
  end

  def remove_person
    persona = Persona.find(params[:person_id])
    @proyecto.personas.delete(persona)
    redirect_to proyectos_path
  end
  ...
  private
    ...
    def proyecto_params
      params.require(:proyecto).permit(:title)
    end
end
